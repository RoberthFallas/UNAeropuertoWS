...
CREATE TABLE IF NOT EXISTS `BD_PF_Grupo3`.`usuarios` (
  ...
  `contrasenna` VARCHAR(45) NOT NULL COMMENT 'Contrseña del usuario (encriptada).',
  `fecha_nacimiento` VARCHAR(45) NULL COMMENT 'Fecha de nacimiento del la persona que se está registrando.',
  `fecha_ingreso` DATE NOT NULL COMMENT 'Fecha en que usuario fue ingresado al sistema.',
  `fecha_modificacion` DATE NULL COMMENT 'Última fecha en que se reaalizó alguna modificación a este usuario.',
  `activo` TINYINT NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `id_UNIQUE` (`id` ASC) ,
